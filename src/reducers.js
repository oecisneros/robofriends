...
const createSimpleReducer = (key, initialState, createPayload) =>
  createReducer(initialState, (action) =>
    action.type === key ? createPayload(action) : null
  );
...
export const searchRobots = createSimpleReducer(
  Constants.CHANGE_SEARCH_FIELD,
  initialStateSearch,
  (action) => ({ searchField: action.payload })
);

export const requestRobots = createReducer(initialStateRobots, (action) => {
  switch (action.type) {
